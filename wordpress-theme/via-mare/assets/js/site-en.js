...
async function loadEnglish(){
 ...
 if(grid)grid.innerHTML=units.map(u=>{const p=(u.photos||[]).sort((a,b)=>(a.sort_order||0)-(b.sort_order||0))[0];return '<article class="suite-card"><div class="suite-image">'+(p?'<img src="'+photoUrl(p.storage_path)+'" alt="'+(VM_EN_NAMES[u.name]||u.name)+'">':'')+'</div><div class="suite-copy"><span class="kicker">APARTMENTS VIA MARE</span><h3>'+(VM_EN_NAMES[u.name]||u.name)+'</h3><p class="suite-meta">'+(VM_EN_META[u.name]||'')+'</p><a class="text-link" href="#">View accommodation →</a></div></article>'}).join("");
